...
def test_data(models, device, data_loader, ind2cat, ind2vad, num_images, result_dir='./', test_type='val', args=None):
    ''' Test models on data 
...
def test_emotic(result_path, model_path, ind2cat, ind2vad, context_norm, body_norm, context_mask_norm, context_seg_norm,
                context_depth_norm, args):
    ...
    '''
    # Prepare models 
    model_context = torch.load(os.path.join(model_path, 'model_context_xu.pth'))
    model_body = torch.load(os.path.join(model_path, 'model_body_xu.pth'))
    model_context_seg = None
    model_context_depth = None
    if args.context_depth == True and args.context_seg == True:
        model_context_seg = torch.load(os.path.join(model_path, 'model_context_seg_xu.pth'))
        model_context_depth = torch.load(os.path.join(model_path, 'model_context_seg_depth.pth'))
    elif args.context_depth == True:
        model_context_depth = torch.load(os.path.join(model_path, 'model_context_seg_depth.pth'))
    elif args.context_seg == True:
        model_context_seg = torch.load(os.path.join(model_path, 'model_context_seg_xu.pth'))

    emotic_model = torch.load(os.path.join(model_path, 'model_emotic_xu.pth'))
    print('Succesfully loaded models')

    # Load data preprocessed npy files

    test_context = np.load(os.path.join(args.data_path, 'test_context_mask_arr.npy'))
    test_context_mask = np.load(os.path.join(args.data_path, 'test_context_arr.npy'))
    ...
    test_context_depth = np.load(os.path.join(args.data_path, 'test_context_depth_arr.npy'))
    test_context_seg = np.load(os.path.join(args.data_path, 'test_context_seg_arr.npy'))
    ...
    test_dataset = Emotic_PreDataset(test_context, test_body, test_context_mask, test_context_seg, test_context_depth,
                                     test_cat, test_cont, test_transform, context_norm, body_norm, context_mask_norm,
                                     context_seg_norm, context_depth_norm)
    test_loader = DataLoader(test_dataset, args.batch_size, shuffle=False)
    print('test loader ', len(test_loader))

    device = torch.device("cuda:%s" % (str(args.gpu)) if torch.cuda.is_available() else "cpu")
    test_data([model_context, model_body, emotic_model, model_context_seg, model_context_depth], device, test_loader,
              ind2cat, ind2vad, len(test_dataset), result_dir=result_path, test_type='test', args=args)
