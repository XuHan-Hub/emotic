...
def parse_args():
    ...
    parser.add_argument('--context_model', type=str, default='resnet18',
                        choices=['resnet18', 'resnet50', 'densenet161'], help='context model type')
    parser.add_argument('--body_model', type=str, default='resnet18', choices=['resnet18', 'resnet50', 'densenet161'],
    ...
    parser.add_argument('--context_depth', type=bool, default=False)
    parser.add_argument('--context_mask', type=bool, default=False)
    parser.add_argument('--context_seg', type=bool, default=False)
    parser.add_argument('--context_ABN', type=bool, default=False)
    # Generate args
...
if __name__ == '__main__':
    ...
    context_mask_mean = [0.4690646, 0.4407227, 0.40508908]
    context_mask_std = [0.2514227, 0.24312855, 0.24266963]
    context_seg_mean = [0.4690646, 0.4407227, 0.40508908]
    context_seg_std = [0.2514227, 0.24312855, 0.24266963]
    context_depth_mean = [0.4690646, 0.4407227, 0.40508908]
    context_depth_std = [0.2514227, 0.24312855, 0.24266963]
    context_norm = [context_mean, context_std]
    body_norm = [body_mean, body_std]
    context_mask_norm = [context_mask_mean, context_mask_std]
    context_seg_norm = [context_seg_mean, context_seg_std]
    context_depth_norm = [context_depth_mean, context_depth_std]
    ...
    if args.mode == 'train':
        ...
        train_emotic(result_path, model_path, train_log_path, val_log_path, ind2cat, ind2vad, context_norm, body_norm,
                     context_mask_norm, context_seg_norm, context_depth_norm, args)
    elif args.mode == 'test':
        if args.data_path is None:
            raise ValueError('Data path not provided. Please pass a valid data path for testing')
        test_emotic(result_path, model_path, ind2cat, ind2vad, context_norm, body_norm, context_mask_norm,
                    context_seg_norm, context_depth_norm, args)
